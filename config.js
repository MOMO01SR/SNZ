...
// 1. نظام التشغيل المتسلسل والمحصن
window.addEventListener('load', () => {
    initApp();
});

async function initApp() {
    // حماية الصفحة والتأكد من وجود مستخدم
    ...
    // تشغيل الـ Fallback (مفتاح الطوارئ)
    // إذا لم ينتهي جلب البيانات خلال 3 ثوانٍ، سيختفي اللودر إجبارياً ليتمكن المستخدم من التصفح
    ...
    try {
        ...
        // إذا نجح الاتصال قبل الـ 3 ثوانٍ، نلغي مفتاح الطوارئ ونخفي اللودر بشكل طبيعي
        if (isDataLoaded) {
        ...
        // إذا حدث خطأ فادح في الجافاسكربت، ننتظر مفتاح الطوارئ ليعمل
        const textEl = document.getElementById('loaderText');
        if (textEl) {
            textEl.innerText = "CONNECTION TIMEOUT. USING CACHE...";
            textEl.classList.replace('text-cyan-400', 'text-yellow-500');
        }
    }
}
...
function hideLoader() {
    ...
    if (loader) {
        ...
        setTimeout(() => {
            ...
            document.body.style.overflow = 'auto'; // مهم جداً لإلغاء تجميد الصفحة
        }, 500);
    }
}

// 3. جلب البيانات من Supabase باحترافية
async function loadUserData() {
    ...
    // التحقق من وجود Supabase أولاً لتجنب الأخطاء القاتلة
    if (typeof _supabase === 'undefined') {
        console.error("Supabase client not loaded. Check config.js.");
        updateDashboardUI(userEmail.split('@')[0], "OFFLINE", "ERROR", 0);
        return false;
    }
    ...
    try {
        ...
        if (data) {
            ...
            if (!currentUID || currentUID === "") {
                ...
                // لن ننتظر تحديث القاعدة لكي لا يعلق الموقع
                _supabase.from('users').update({ uid: currentUID }).eq('email', userEmail);
            }
        ...
        // في حال فشل الاتصال نعرض بيانات افتراضية لكي لا تتعطل الشاشة
        updateDashboardUI(userEmail.split('@')[0], "OFFLINE", "N/A", 0);
        return false;
    }
}

// 4. تحديث الواجهة والأنيميشن
...
async function executeSignal() {
    ...
    const btn = document.getElementById('signalStep1');
    if(btn) btn.classList.add('hidden');
    
    const investZone = document.getElementById('investmentZone');
    if(investZone) investZone.classList.add('hidden');
    
    const tradeUI = document.getElementById('tradeUI');
    if(tradeUI) tradeUI.classList.remove('hidden');
...
async function finalizeTrade(code, invest) {
    if (typeof _supabase === 'undefined') return alert("حدث خطأ في الاتصال بالخادم.");
    try {
